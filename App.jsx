...
const ST = {
  pending: { color: "#00d97e", bg: "rgba(0,217,126,0.08)",   label: "Aguardando" },
  sending: { color: "#f5a623", bg: "rgba(245,166,35,0.08)",  label: "Enviando…"  },
  sent:    { color: "#4a9eff", bg: "rgba(74,158,255,0.08)",  label: "Enviado"    },
...
function SlotCard({ slot, index, onChange, onRemove, canRemove }) {
  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16, padding: 20, marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ width: 24, height: 24, borderRadius: 8, background: "rgba(74,158,255,0.12)", border: "1px solid rgba(74,158,255,0.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: "#4a9eff", fontFamily: "monospace" }}>{index + 1}</div>
          <span style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.3)", letterSpacing: 1.5, textTransform: "uppercase", fontFamily: "monospace" }}>Disparo {index + 1}</span>
        </div>
...
function QRModal({ instanceId, apiUrl, apiKey, onClose, onConnected }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.88)", backdropFilter: "blur(12px)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#0d0f1a", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 24, padding: 32, maxWidth: 360, width: "90%", textAlign: "center" }}>
        ...
        <div style={{ fontSize: 16, fontWeight: 700, color: "#fff", marginBottom: 4 }}>Conectar WhatsApp</div>
        <div style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", fontFamily: "monospace", marginBottom: 24 }}>{instanceId}</div>
        {status === "loading" && <div style={{ color: "rgba(255,255,255,0.25)", fontFamily: "monospace", fontSize: 12, padding: 40 }}>Carregando QR Code...</div>}
        {status === "qr" && qr && <><div style={{ background: "#fff", borderRadius: 14, padding: 10, display: "inline-block", marginBottom: 14 }}><img src={qr} alt="QR" style={{ width: 200, height: 200, display: "block" }} /></div><div style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", fontFamily: "monospace" }}>WhatsApp → Aparelhos conectados</div></>}
        ...
        <button onClick={onClose} style={{ display: "block", width: "100%", marginTop: 12, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", color: "rgba(255,255,255,0.35)", borderRadius: 10, padding: "10px", cursor: "pointer", fontFamily: "monospace", fontSize: 12 }}>Fechar</button>
      </div>
    </div>
  );
}
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        html,body{background:#07080f;min-height:100vh;}
        ...
        input:focus,textarea:focus,select:focus{outline:none!important;border-color:rgba(74,158,255,.45)!important;box-shadow:0 0 0 3px rgba(74,158,255,.07)!important;}
        textarea{resize:vertical;}
        ::-webkit-scrollbar{width:3px;}
        ::-webkit-scrollbar-thumb{background:rgba(255,255,255,.07);border-radius:4px;}
        .tab-btn:hover{background:rgba(255,255,255,.04)!important;color:rgba(255,255,255,.6)!important;}
        .rm-btn:hover{color:rgba(255,77,77,.7)!important;}
        .del-btn:hover{color:rgba(255,77,77,.7)!important;}
        .inst-card:hover{border-color:rgba(255,255,255,.1)!important;}
      `}</style>

      <div style={{ minHeight: "100vh", background: "#07080f", fontFamily: "'Plus Jakarta Sans', sans-serif", color: "#fff", display: "flex" }}>

        {/* Sidebar */}
        <div style={{ width: 224, flexShrink: 0, background: "rgba(255,255,255,0.015)", borderRight: "1px solid rgba(255,255,255,0.04)", display: "flex", flexDirection: "column", position: "sticky", top: 0, height: "100vh" }}>
          <div style={{ padding: "28px 20px 20px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 2 }}>
              <div style={{ width: 36, height: 36, borderRadius: 11, background: "linear-gradient(135deg,rgba(0,217,126,0.15),rgba(74,158,255,0.15))", border: "1px solid rgba(74,158,255,0.18)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>📲</div>
              <div>
                <div style={{ fontSize: 13, fontWeight: 800, color: "#fff", letterSpacing: -0.2 }}>AutoDisparo</div>
                <div style={{ fontSize: 9, color: "rgba(255,255,255,0.2)", fontFamily: "'DM Mono',monospace", letterSpacing: 1 }}>WHATSAPP · v2.0</div>
              </div>
            </div>
          </div>
          ...
          <nav style={{ flex: 1, padding: "0 12px" }}>
            {TABS.map(t => (
              <button key={t.id} className="tab-btn" onClick={() => setTab(t.id)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: tab === t.id ? "rgba(74,158,255,0.07)" : "none", border: `1px solid ${tab === t.id ? "rgba(74,158,255,0.14)" : "transparent"}`, borderRadius: 10, cursor: "pointer", color: tab === t.id ? "#4a9eff" : "rgba(255,255,255,0.3)", fontSize: 13, fontWeight: tab === t.id ? 600 : 400, fontFamily: "'Plus Jakarta Sans',sans-serif", transition: "all .18s", marginBottom: 3, textAlign: "left" }}>
                <span style={{ fontSize: 13, opacity: tab === t.id ? 1 : 0.5 }}>{t.icon}</span>
                {t.label}
                {t.id === "schedule" && pending > 0 && <span style={{ marginLeft: "auto", background: "#4a9eff", color: "#fff", borderRadius: 20, padding: "1px 7px", fontSize: 9, fontWeight: 700 }}>{pending}</span>}
          ...
          <div style={{ padding: "16px 20px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <div style={{ width: 6, height: 6, borderRadius: "50%", background: config.url ? "#00d97e" : "rgba(255,255,255,0.1)", boxShadow: config.url ? "0 0 8px #00d97e88" : "none" }} />
              <span style={{ fontSize: 9, color: "rgba(255,255,255,0.2)", fontFamily: "'DM Mono',monospace", letterSpacing: 0.8 }}>{config.url ? "API CONECTADA" : "SEM CONFIG"}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ padding: "32px 36px 24px", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
            <h1 style={{ fontSize: 20, fontWeight: 700, color: "#fff", marginBottom: 3 }}>{TABS.find(t => t.id === tab)?.label}</h1>
          ...
          <div style={{ padding: "28px 36px 60px", maxWidth: 720 }}>
              ...
              <div className="fade-up">
                <Sec label="Produto">
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10 }}>
                    {PRODUCTS.map(p => (
                      <div key={p.id} onClick={() => setForm(f => ({ ...f, product: p.id }))} style={{ border: `1px solid ${form.product === p.id ? p.accent + "40" : "rgba(255,255,255,0.05)"}`, background: form.product === p.id ? p.glow : "rgba(255,255,255,0.015)", borderRadius: 14, padding: "16px 10px", cursor: "pointer", textAlign: "center", transition: "all .2s" }}>
                        <div style={{ fontSize: 26, marginBottom: 8 }}>{p.emoji}</div>
                        <div style={{ fontSize: 11, fontWeight: 600, color: form.product === p.id ? p.accent : "rgba(255,255,255,0.25)", letterSpacing: 0.3 }}>{p.label}</div>
                      </div>
                    ))}
                  </div>
                </Sec>
                ...
                <Sec label="WhatsApp Remetente">
                  {connected.length === 0
                    ? <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 12, padding: "13px 16px", fontSize: 11, color: "rgba(255,255,255,0.2)", fontFamily: "'DM Mono',monospace" }}>Nenhum número conectado · Vá em <span style={{ color: "#4a9eff", cursor: "pointer" }} onClick={() => setTab("whatsapp")}>WhatsApp</span></div>
                    : <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>{connected.map(inst => <div key={inst.id} onClick={() => setForm(f => ({ ...f, sender: inst.id }))} style={{ border: `1px solid ${form.sender === inst.id ? "#00d97e40" : "rgba(255,255,255,0.06)"}`, background: form.sender === inst.id ? "rgba(0,217,126,0.06)" : "rgba(255,255,255,0.02)", borderRadius: 10, padding: "9px 16px", cursor: "pointer", transition: "all .18s" }}><div style={{ fontSize: 11, fontWeight: 600, color: form.sender === inst.id ? "#00d97e" : "rgba(255,255,255,0.25)", fontFamily: "'DM Mono',monospace" }}>📱 {inst.label}</div></div>)}</div>
                  }
                </Sec>
                ...
                <Sec label="Telefones · um por linha com DDI">
                  ...
                  {form.phones.trim() && <div style={{ marginTop: 5, fontSize: 10, color: "rgba(255,255,255,0.18)", fontFamily: "'DM Mono',monospace" }}>{form.phones.split("\n").filter(p => p.trim()).length} contato(s)</div>}
                </Sec>

                <Sec label={`Disparos · ${form.slots.length}/3`} action={form.slots.length < 3 && <button onClick={() => setForm(f => ({ ...f, slots: [...f.slots, eSlot()] }))} style={{ background: "rgba(74,158,255,0.08)", border: "1px solid rgba(74,158,255,0.18)", color: "#4a9eff", borderRadius: 8, padding: "5px 12px", cursor: "pointer", fontSize: 10, fontFamily: "'DM Mono',monospace", fontWeight: 700 }}>+ Adicionar</button>}>
            ...
            {tab === "schedule" && (
              <div className="fade-up">
                {projects.length === 0
                  ? <div style={{ textAlign: "center", padding: "80px 20px" }}><div style={{ fontSize: 32, color: "rgba(255,255,255,0.05)", marginBottom: 12 }}>◷</div><div style={{ fontSize: 12, color: "rgba(255,255,255,0.12)", fontFamily: "'DM Mono',monospace" }}>Nenhum projeto criado</div></div>
                  : projects.map(proj => {
                    const p = PRODUCTS.find(x => x.id === proj.product);
                    const projD = dispatches.filter(d => d.projectId === proj.id);
                    return (
                      <div key={proj.id} className="inst-card" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 18, marginBottom: 14, overflow: "hidden", transition: "border-color .2s" }}>
                        <div style={{ padding: "14px 18px", borderBottom: "1px solid rgba(255,255,255,0.04)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            ...
                            <div>
                              <div style={{ fontWeight: 700, fontSize: 13, color: "#fff" }}>{p.label}</div>
                              <div style={{ fontSize: 10, color: "rgba(255,255,255,0.2)", fontFamily: "'DM Mono',monospace" }}>📱 {proj.sender} · {proj.phones.length} contato(s)</div>
                            </div>
                          </div>
                          <button className="del-btn" onClick={() => deleteProject(proj.id)} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.12)", cursor: "pointer", fontSize: 15, transition: "color .2s" }}>✕</button>
                        </div>
                        {projD.map(d => (
                          <div key={d.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 18px", borderBottom: "1px solid rgba(255,255,255,0.025)" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                              <span style={{ fontSize: 13 }}>{d.imageB64 ? "🖼️" : "💬"}</span>
                              <div>
                                <div style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.6)", fontFamily: "'DM Mono',monospace" }}>{d.sendLabel}</div>
                                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.18)", fontFamily: "'DM Mono',monospace" }}>{d.imageB64 && d.message ? "foto + msg" : d.imageB64 ? "foto" : "mensagem"}{d.sentAt && <span style={{ color: "#00d97e", marginLeft: 8 }}>· {d.sent} ok{d.failed ? `, ${d.failed} ✕` : ""}</span>}</div>
                              </div>
                            </div>
            ...
            {tab === "whatsapp" && (
              <div className="fade-up">
                <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 20 }}>
                  <button onClick={refreshStatus} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.35)", borderRadius: 10, padding: "8px 16px", cursor: "pointer", fontSize: 11, fontFamily: "'DM Mono',monospace", transition: "all .2s" }}>↻ Sincronizar</button>
                </div>
                {instances.map(inst => (
                  <div key={inst.id} className="inst-card" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 16, padding: "15px 18px", marginBottom: 10, display: "flex", alignItems: "center", justifyContent: "space-between", transition: "border-color .2s" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <div>
                        <div style={{ fontWeight: 600, fontSize: 13, color: "#fff", marginBottom: 3 }}>{inst.label}</div>
                        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                          <div style={{ width: 5, height: 5, borderRadius: "50%", background: inst.status === "connected" ? "#00d97e" : "rgba(255,255,255,0.12)", boxShadow: inst.status === "connected" ? "0 0 6px #00d97e" : "none" }} />
                          <span style={{ fontSize: 9, color: inst.status === "connected" ? "#00d97e" : "rgba(255,255,255,0.2)", fontFamily: "'DM Mono',monospace", letterSpacing: 0.8 }}>{inst.status === "connected" ? "CONECTADO" : "DESCONECTADO"}</span>
                        </div>
                      </div>
                    </div>
                    <div style={{ display: "flex", gap: 8 }}>
                      {inst.status !== "connected" && <button onClick={() => setQrModal(inst.id)} style={{ background: "rgba(0,217,126,0.07)", border: "1px solid rgba(0,217,126,0.18)", color: "#00d97e", borderRadius: 9, padding: "7px 13px", cursor: "pointer", fontSize: 10, fontFamily: "'DM Mono',monospace", fontWeight: 700 }}>QR Code</button>}
                      <button className="del-btn" onClick={() => setInstances(prev => prev.filter(i => i.id !== inst.id))} style={{ background: "none", border: "1px solid rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.18)", borderRadius: 9, padding: "7px 11px", cursor: "pointer", fontSize: 12, transition: "color .2s" }}>✕</button>
                    </div>
                  </div>
                ))}
                {instances.length < 5 && (
                  <div style={{ background: "rgba(255,255,255,0.01)", border: "1.5px dashed rgba(255,255,255,0.05)", borderRadius: 16, padding: 24, textAlign: "center" }}>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.15)", fontFamily: "'DM Mono',monospace", marginBottom: 14 }}>{instances.length}/5 números configurados</div>
                    <button onClick={() => addInstance(`WA-${instances.length + 1}`)} style={{ background: "linear-gradient(135deg,rgba(0,217,126,0.1),rgba(74,158,255,0.1))", border: "1px solid rgba(74,158,255,0.18)", color: "#fff", borderRadius: 12, padding: "11px 28px", cursor: "pointer", fontSize: 13, fontWeight: 600, fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
            ...
            {tab === "config" && (
              <div className="fade-up">
                <div style={{ background: "rgba(74,158,255,0.04)", border: "1px solid rgba(74,158,255,0.1)", borderRadius: 14, padding: 18, marginBottom: 24 }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: "#4a9eff", fontFamily: "'DM Mono',monospace", letterSpacing: 1, marginBottom: 8 }}>EVOLUTION API</div>
                  <div style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", fontFamily: "'DM Mono',monospace", lineHeight: 1.9 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  ...
                  <button onClick={() => showToast("✓ Configurações salvas")} style={{ padding: 14, background: "rgba(74,158,255,0.08)", border: "1px solid rgba(74,158,255,0.18)", color: "#4a9eff", borderRadius: 13, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "'Plus Jakarta Sans',sans-serif", letterSpacing: 0.2 }}>Salvar Configurações</button>
                </div>
              </div>
            )}
  ...
  return (
    <div style={{ marginBottom: 26 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
        <span style={{ fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,0.2)", letterSpacing: 1.8, textTransform: "uppercase", fontFamily: "'DM Mono',monospace" }}>{label}</span>
        {action}
      </div>
      {children}
    </div>
...
function SLabel({ children }) {
  return <div style={{ fontSize: 9, color: "rgba(255,255,255,0.2)", fontFamily: "'DM Mono',monospace", fontWeight: 700, letterSpacing: 1.2, marginBottom: 6, textTransform: "uppercase" }}>{children}</div>;
}

const iSt = {
  width: "100%", background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.07)",
  color: "#fff", borderRadius: 12, padding: "11px 14px", fontSize: 12, transition: "all .2s"
};
